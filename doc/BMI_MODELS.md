...
Because of the use of `iso_c_bindings`, integrating with a Fortran BMI module operate very much like integrating with C BMI module as a [shared library](#bmi-c-model-as-shared-library) that is [dynamically loaded](#dynamic-loading).  An extra [bootstrapping registration function](#required-additional-fortran-registration-function) is also, again, required.
