...
This directory wraps the *CFE** Git submodule repo, which contains a BMI implementation of the Concetupal Functional Equivalent (CFE) model.  From here, a shared library file for *CFE* can be built for use in NGen.  This is configured with the [CMakeLists.txt](CMakeLists.txt) and other files in this outer directory.
...
To fetch and check out the latest revision (for the [currently used branch](#viewing-the-current-branch)):
    ...
    git submodule update --init --remote -- extern/cfe/cfe
...
Before the library file can be built, a CMake build system must be generated.  E.g.:

    cmake -B cmake_cfe_lib -S .
...
After there is build system directory, the shared library can be built. The CFE shared library file (i.e., the build config's `cfebmi` target) can be built using:

    cmake --build cmake_cfe_lib --target cfebmi -- -j 2

This will build a `cmake_cfe_lib/libcfebmi.<version>.<ext>` file, where the version is configured within the CMake config, and the extension depends on the local machine's operating system.    
