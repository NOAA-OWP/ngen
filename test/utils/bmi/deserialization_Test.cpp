...
TEST_F(Bmi_Deserialization_Test, id_subset_matches_compound_prefix) {
    ...
    append_record(path, SerializationRecord{record_id, 0, int64_t{0},
                                            make_payload(7.5, 1.0, 2.0, 3.0, 4.0)});
    ...
    auto result = protocols.run(Protocol::DESERIALIZATION,
                                make_context(0, 2, "t0", record_id));
    ASSERT_TRUE(result.has_value());
...
TEST_F(Bmi_Deserialization_Test, id_subset_matches_multi_submodule_prefix) {
    ...
    append_record(path, SerializationRecord{record_id, 0, int64_t{0},
                                            make_payload(11.0, 1.0, 2.0, 3.0, 4.0)});
