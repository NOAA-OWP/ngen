...
std::string Bmi_Cpp_Test_Adapter_For_Deserialization::file_search(
...
TEST_F(Bmi_Deserialization_Test, disabled) {
    ...
    append_record(
        path,
        SerializationRecord{model_name, 0, int64_t{0}, make_payload(0, 1, 2, 3, 4)}
    );
...
TEST_F(Bmi_Deserialization_Test, missing_path) {
    ...
    EXPECT_THAT(
        output,
        testing::HasSubstr("Warning(Protocol)::deserialization: 'path' not specified")
    );
}
...
TEST_F(Bmi_Deserialization_Test, restore_fixed_step) {
    ...
    append_record(
        path,
        SerializationRecord{model_name, 0, int64_t{0}, make_payload(0.0, 1.0, 2.0, 3.0, 4.0)}
    );
    append_record(
        path,
        SerializationRecord{"other", 0, int64_t{0}, make_payload(999.0, 9.0, 9.0, 9.0, 9.0)}
    );
    append_record(
        path,
        SerializationRecord{model_name, 5, int64_t{300}, make_payload(500.0, 5.0, 6.0, 7.0, 8.0)}
    );
...
TEST_F(Bmi_Deserialization_Test, id_subset_matches_compound_prefix) {
    // Subset entries name just the primary identity ("cat-1"), while
    // record ids may carry one or more qualifiers separated by `:`
    // ("cat-1:<some-qualifier>"). The index filter must normalize to
    // the primary-identity prefix on both sides; comparing the full
    // record id against a primary-only subset entry would filter the
    // record out.
    const std::string record_id = "cat-1:" + model_name;
    append_record(
        path,
        SerializationRecord{record_id, 0, int64_t{0}, make_payload(7.5, 1.0, 2.0, 3.0, 4.0)}
    );

    // Subset entry is the primary identity only.
    auto properties =
        DeserializationMock(path, /*step*/ "latest").with_id_subset({"cat-1"}).as_json_property();
    auto protocols = NgenBmiProtocols(model, properties);

    // Lookup still uses the full record id (`by_id_` is keyed by the
    // full string; only the build-time subset filter normalizes).
    auto result = protocols.run(Protocol::DESERIALIZATION, make_context(0, 2, "t0", record_id));
    ASSERT_TRUE(result.has_value());
...
TEST_F(Bmi_Deserialization_Test, id_subset_matches_multi_submodule_prefix) {
    // Record ids can also use `.` as a sub-index delimiter:
    // "primary.<sub>:<qualifier>...". Verify the prefix split treats
    // `.` and `:` symmetrically as primary-identity terminators.
    const std::string record_id = "cat-1.2:" + model_name + ":multi_owner";
    append_record(
        path,
        SerializationRecord{record_id, 0, int64_t{0}, make_payload(11.0, 1.0, 2.0, 3.0, 4.0)}
    );
...
TEST_F(Bmi_Deserialization_Test, id_subset_does_not_match_unrelated_prefix) {
    // Whole-token prefix comparison: "cat-1" must not match a record
    // whose primary-identity prefix is "cat-10" (no accidental
    // string-prefix overlap). The split happens at the first delimiter,
    // not at the subset entry's length, so prefixes are compared as
    // whole tokens.
    const std::string record_id = "cat-10:" + model_name;
    append_record(path, SerializationRecord{record_id, 0, int64_t{0}, make_payload(0, 1, 2, 3, 4)});

    auto properties = DeserializationMock(path, /*step*/ "latest", /*fatal*/ false)
                          .with_id_subset({"cat-1"})
                          .as_json_property();
    auto protocols = NgenBmiProtocols(model, properties);
    testing::internal::CaptureStderr();
    auto result = protocols.run(Protocol::DESERIALIZATION, make_context(0, 2, "t0", record_id));
...
TEST_F(Bmi_Deserialization_Test, missing_id_warns) {
    ...
    append_record(
        path,
        SerializationRecord{"some-other-id", 0, int64_t{0}, make_payload(0, 1, 2, 3, 4)}
    );

    auto properties =
        DeserializationMock(path, /*step*/ "latest", /*fatal*/ false).as_json_property();
    auto protocols = NgenBmiProtocols(model, properties);
    auto result    = protocols.run(Protocol::DESERIALIZATION, make_context(0, 2, "t0", model_name));
    ASSERT_FALSE(result.has_value());
    EXPECT_THAT(
        result.error().to_string(),
        testing::HasSubstr("Warning(Protocol)::deserialization:")
    );
...
TEST_F(Bmi_Deserialization_Test, missing_id_throws) {
    ...
    auto properties =
        DeserializationMock(path, /*step*/ "latest", /*fatal*/ true).as_json_property();
    auto protocols = NgenBmiProtocols(model, properties);
    ASSERT_THROW(
        (void)protocols.run(Protocol::DESERIALIZATION, make_context(0, 2, "t0", model_name)),
        ProtocolError
    );
}
...
TEST_F(Bmi_Deserialization_Test, restore_by_timestamp) {
    ...
    append_record(
        ...
        SerializationRecord{
            ...
            0,
            int64_t{1448928000},
            make_payload(0.0, 1.0, 2.0, 3.0, 4.0)
        }
    );
    append_record(
        path,
        SerializationRecord{
            ...
            1,
            int64_t{1448931600},
            make_payload(100.0, 11., 12., 13., 14.)
        }
    );
    append_record(
        path,
        SerializationRecord{
            ...
            2,
            int64_t{1448935200},
            make_payload(200.0, 21., 22., 23., 24.)
        }
    );
...
TEST_F(Bmi_Deserialization_Test, restore_timestamp_wins_over_step) {
    ...
    auto properties =
        DeserializationMock::both(path, /*step*/ "5", /*timestamp*/ "0").as_json_property();
    auto protocols = NgenBmiProtocols(model, properties);
    auto result =
        protocols.run(Protocol::DESERIALIZATION, make_context(0, 0, "unused", model_name));
    ASSERT_TRUE(result.has_value());
...
TEST_F(Bmi_Deserialization_Test, missing_timestamp_warns) {
    ...
    append_record(
        path,
        SerializationRecord{model_name, 0, int64_t{1448928000}, make_payload(0, 1, 2, 3, 4)}
    );

    auto properties =
        DeserializationMock::by_timestamp(path, "1577836800", /*fatal*/ false).as_json_property();
    auto protocols = NgenBmiProtocols(model, properties);
    auto result =
        protocols.run(Protocol::DESERIALIZATION, make_context(0, 0, "unused", model_name));
    ASSERT_FALSE(result.has_value());
...
TEST_F(Bmi_Deserialization_Test, save_restore_roundtrip) {
    ...
    auto restore_r =
        protocols.run(Protocol::DESERIALIZATION, make_context(0, 0, "restore", model_name));
...
TEST_F(Bmi_Deserialization_Test, save_restore_roundtrip_by_timestamp) {
    ...
    auto properties                  = DeserializationMock::by_timestamp(path, save_timestamp)
    ...
    auto save_r =
        protocols.run(Protocol::SERIALIZATION, make_context(1, 3, save_timestamp, model_name));
    ASSERT_TRUE(save_r.has_value());
