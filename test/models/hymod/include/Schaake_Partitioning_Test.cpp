...
void SchaakeKernelTest::SetUp() {
    setupArbitraryExampleCase();
}

void SchaakeKernelTest::TearDown() {

}

void SchaakeKernelTest::setupArbitraryExampleCase() {

}


// Test that schaake_partitioning_scheme(timestep_s,
...
// run correctly when passed known input
TEST_F(SchaakeKernelTest, TestSchaakePartitioning)
{
...
int    num_soil_discretizations;                        // nsoil+1; using vector convention
double timestep_s;                                      // dt;
//double *soil_discretization_thickness_m;              // *dz; now array allocated using vector
//double *soil_volumetric_moisture_content;             // *sh2o; now array allocated using vector
...
//double *soil_max_water_storage_per_discretization_m;  // dmax; now array allocated using vector
...
num_soil_discretizations=5;

//ALLOCATE MEMORY FOR THESE FOUR ARRAYS
std::vector<double> *soil_discretization_thickness_m = new std::vector<double>();
d_alloc(soil_discretization_thickness_m,num_soil_discretizations);
std::vector<double> *soil_volumetric_moisture_content = new std::vector<double>();
d_alloc(soil_volumetric_moisture_content,num_soil_discretizations);
std::vector<double> *soil_max_water_storage_per_discretization_m = new std::vector<double>();
d_alloc(soil_max_water_storage_per_discretization_m,num_soil_discretizations);

/* the thickness of the discretizations */
soil_discretization_thickness_m->at(0)=0.0;   // NOTE: it is strange that they used negative signs here, normally in soils physics,
soil_discretization_thickness_m->at(1)=0.10;  //       people create a coordinate system that is positive downward. -FLO
soil_discretization_thickness_m->at(2)=0.30;  //       The end result is that the math below is weird in calculating the thickness
soil_discretization_thickness_m->at(3)=0.60;  //        of the various discretizations, which are actually already known.
soil_discretization_thickness_m->at(4)=1.00;  //          I define these as POSITIVE real numbers.
...
soil_volumetric_moisture_content->at(0) = 0.0;
soil_volumetric_moisture_content->at(1) = 0.285762608;
soil_volumetric_moisture_content->at(2) = 0.292563140;
soil_volumetric_moisture_content->at(3) = 0.272871971;
soil_volumetric_moisture_content->at(4) = 0.306821257;
...
soil_max_water_storage_per_discretization_m->at(1)= soil_discretization_thickness_m->at(1) * soil_max_avail_moisture_capacity;                        // max.depth of water storage [m] in upper most soil discretization
soil_max_water_storage_per_discretization_m->at(1)= soil_max_water_storage_per_discretization_m->at(1)* (1.0-(soil_volumetric_moisture_content->at(1) - soil_wilting_point_moisture_content)/soil_max_avail_moisture_capacity);  // Actual water storage deficit [m] in uppermost soil discretization
...
column_total_soil_moisture_deficit_m = soil_max_water_storage_per_discretization_m->at(1);   // Soil moisture deficit (m) sum, initialized to start the summation in the next loop.

for(k = 2;k<num_soil_discretizations;k++)
  {
  soil_max_water_storage_per_discretization_m->at(k) = soil_discretization_thickness_m->at(k) * soil_max_avail_moisture_capacity;          // max. depth of moisture storage in kth discretization
  soil_max_water_storage_per_discretization_m->at(k) = soil_max_water_storage_per_discretization_m->at(k) * (1.0-(soil_volumetric_moisture_content->at(k) - soil_wilting_point_moisture_content)/soil_max_avail_moisture_capacity); // Actual water storage deficit [m] in kth discretization, dmax[k]*(1-(a ratio from 0 to 1))
  column_total_soil_moisture_deficit_m  += soil_max_water_storage_per_discretization_m->at(k);        // Sum of available water storage in entire profile [m].
  }
