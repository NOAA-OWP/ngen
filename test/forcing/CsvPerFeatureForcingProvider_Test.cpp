...
TEST_F(CsvPerFeatureForcingProviderTest, TestForcingDataRead)
{
    ...
    current_precipitation = Forcing_Object->get_value(CatchmentAggrDataSelector("", CSDMS_STD_NAME_LIQUID_EQ_PRECIP_RATE, begin+(i*3600), 3600, "mm/s", 0), data_access::SUM);

    EXPECT_NEAR(current_precipitation, 7.9999999999999996e-07, 0.00000005);

    double temp_k = Forcing_Object->get_value(CatchmentAggrDataSelector("", CSDMS_STD_NAME_SURFACE_TEMP, begin+(i*3600), 3600, "K", 0), data_access::MEAN);
...
TEST_F(CsvPerFeatureForcingProviderTest, TestForcingDataReadAltFormat)
{
    ...
    current_precipitation = Forcing_Object_2->get_value(CatchmentAggrDataSelector("", CSDMS_STD_NAME_LIQUID_EQ_PRECIP_RATE, begin+(i*3600), 3600, "mm/s", 0), data_access::SUM);

    EXPECT_NEAR(current_precipitation, 0.00032685, 0.00000001);

    double temp_k = Forcing_Object_2->get_value(CatchmentAggrDataSelector("", CSDMS_STD_NAME_SURFACE_TEMP, begin+(i*3600), 3600, "K", 0), data_access::MEAN);
...
TEST_F(CsvPerFeatureForcingProviderTest, TestForcingDataUnitConversion)
{
    ...
    double temp_f = Forcing_Object->get_value(CatchmentAggrDataSelector("", CSDMS_STD_NAME_SURFACE_TEMP, begin+(i*3600), 3600, "degF", 0), data_access::MEAN);
