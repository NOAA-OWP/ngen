...
double Bmi_Fortran_Adapter::GetTimeStep() {
    ...
    if (get_time_step(&bmi_model->handle, &ts) != BMI_SUCCESS) {
        Logger::logMsgAndThrowError(model_name + " failed to get model time step size.");
    }
    return ts;
}
...
void Bmi_Fortran_Adapter::GetValueAtIndices(std::string name, void *dest, int *inds, int count) {
     ...
     */
    Logger::logMsgAndThrowError("Fortran BMI module integration does not currently support getting values by index");
}
...
void Bmi_Fortran_Adapter::SetValueAtIndices(std::string name, int *inds, int count, void *src) {
    ...
    */
    Logger::logMsgAndThrowError("Fortran BMI module integration does not currently support setting values by index");
}
