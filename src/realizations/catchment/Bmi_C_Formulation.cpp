...
Bmi_C_Formulation::Bmi_C_Formulation(std::string id, std::shared_ptr<data_access::GenericDataProvider> forcing_provider, utils::StreamHandler output_stream)
    : Bmi_Module_Formulation<models::bmi::Bmi_C_Adapter>(id, forcing_provider, output_stream) { }
...
std::string Bmi_C_Formulation::get_output_line_for_timestep(int timestep, std::string delimiter) {
    ...
    std::cout << "enter get_output_line_for_timestep " << std::endl;
...
std::vector<double> Bmi_C_Formulation::get_var_vec_as_double(time_t t_delta, const string &var_name) {
...
bool Bmi_C_Formulation::is_bmi_output_variable(const std::string &var_name) {
