...
void Bmi_C_Formulation::inner_create_formulation(geojson::PropertyMap properties, bool needs_param_validation) {
    ...
    // Do this first, since we can construct the adapter and init the model as soon as we know whether it's present
    ...
    if (other_in_var_it != properties.end()) {
        ...
        set_bmi_model(std::make_shared<Bmi_C_Adapter>(
                Bmi_C_Adapter(get_bmi_init_config(), get_forcing_file_path(), is_bmi_uses_forcing_file(),
                              other_in_var_it->second, output)));
    }
    else {
        set_bmi_model(std::make_shared<Bmi_C_Adapter>(
                Bmi_C_Adapter(get_bmi_init_config(), get_forcing_file_path(), is_bmi_uses_forcing_file(), output)));
    }
...
double Bmi_C_Formulation::get_response(time_step_t t_index, time_step_t t_delta) {
    ...
    std::string type = get_bmi_model()->GetVarType(get_bmi_main_output_var());
    if (type == "double") {
        std::vector<double> outputs = get_bmi_model()->GetValue<double>(get_bmi_main_output_var());
        return outputs[t_index];
    }
    else if (type == "float") {
        std::vector<float> outputs = get_bmi_model()->GetValue<float>(get_bmi_main_output_var());
        return (double)(outputs[t_index]);
    }
    else if (type == "int") {
        std::vector<int> outputs = get_bmi_model()->GetValue<int>(get_bmi_main_output_var());
        return (double)(outputs[t_index]);
    }
    else if (type == "long") {
        std::vector<long> outputs = get_bmi_model()->GetValue<long>(get_bmi_main_output_var());
        return (double)(outputs[t_index]);
    }
}
