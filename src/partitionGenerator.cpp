...
int main(int argc, char* argv[])
{
    ...
    //
    // Mirror the terminal-nexus sentinel loop above: attach a per-catchment
    // 'wb-OUTLET_SENTINEL-<cat_id>' SentinelFeature as destination so
    // partitioning can run. These sentinels aren't picked up by
    // network.filter("cat", ...), so ngen never simulates them.
