...
//############################################################*
double et::combined::evapotranspiration_combination_method
        (
                et::evapotranspiration_options *et_options,
                et::evapotranspiration_params *et_params,
                et::evapotranspiration_forcing *et_forcing,
                et::intermediate_vars *inter_vars
        ) {
    // local variables
    ...
    if ((et_options->use_aerodynamic_method == FALSE) && (et_options->use_penman_monteith_method == FALSE)) {
        ...
        mass_flux =
                0.622 * von_karman_constant_squared * moist_air_density_kg_per_m3 *// kg per sq. meter per sec.
                vapor_pressure_deficit_Pa * et_forcing->wind_speed_m_per_s /
                (et_forcing->air_pressure_Pa *
                 pow(log(et_params->wind_speed_measurement_height_m /
                         et_params->zero_plane_displacement_height_m), 2.0));
