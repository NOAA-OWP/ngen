...
auto NgenSerializationProtocol::run(const ModelPtr& model, const Context& ctx) const
    ...
    try {
        ...
        // Acquire a Writer, hand it a non-owning `RecordView` over
        // the function-scoped payload buffer, write, commit, drop.
        // The Writer handle is a thin intent carrier for
        // durability + lifetime; the backend owns the underlying
        // I/O resources and serializes writes across any
        // concurrent Writers. See `file_backend.hpp` for the
        // FileBackend-specific mechanics.
