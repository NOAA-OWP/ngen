...
**v0.1.0** — initial release. Version constants live in
[`version.hpp`](version.hpp); consumers depending on a specific
release's features can `static_assert` on them.

The 0.X series makes no API-compatibility guarantee. The abstract
`RecordBackend` surface, `BackendError::Kind`, and the `IdPredicate`
kit are all subject to revision as concrete backends inform the
design. Pin to an exact 0.X.Y release and expect to update per
release. At 1.0.0, the API will be stable, and
MAJOR / MINOR / PATCH will carry their conventional Semantic Versioning meanings
from that point on.
...
**Not in v0.1**:
  ...
  - **Snapshot composition** — how multiple subsystems' state
    ...
    `Snapshot` / `SnapshotCoordinator` layer is under
    consideration.
...
### 3.8 Concurrent sub-handle access

Backends MUST support multiple `Reader` sub-handles alive
concurrently on the same `RecordBackend` instance, including from
different threads. Writers do not compose concurrently — §3.4's
single-in-flight rule is the mechanism.
