...
See `wire_format.hpp` for the rationale behind the chosen
byte-level layout (cross-host portability, library independence,
...
the id bytes, or the payload bytes — which is what
restart-from-crash workflows depend on.
 ...
 * drive `read_next_record` into a multi-gigabyte allocation. The
 * cap exists to reject obvious garbage.
 *
 ...
 * for larger payloads. The default is 1 GiB; the wire-format field
 * type (uint64) supports values up to ~18 EiB.
 */
