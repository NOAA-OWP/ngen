...
class NgenMassBalance : public NgenBmiProtocol {
  ...
  private:
    ...
    /**
     * @brief Check the model for support and initialize the mass balance protocol from the given
     * properties.
     *
     * If the model does not support the mass balance protocol, an exception will be thrown, and no
     * mass balance will performed when `run()` is called.
     *
     ...
     *                      frequency: int, default 1. How often (in time steps) to check mass
     * balance. fatal: bool, default false. Whether to treat mass balance errors as fatal.
